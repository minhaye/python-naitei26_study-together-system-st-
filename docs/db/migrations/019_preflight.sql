-- Preflight checks for 019_create_group_notes.sql.
...
--
-- Context: 019 creates a brand-new `group_notes` table (no pre-existing
...
--
-- Run this BEFORE 019_create_group_notes.sql.

with
  required_tables_exist as (
    select
      'required_tables_exist (must be OK -- groups/profiles, the tables 019''s new FKs point at)' as check_name,
  ...
  group_notes_table_absent as (
    select
      'group_notes_table_absent (must be OK/none-found -- 019 creates this table; if it already exists, STOP and review before running 019)' as check_name,
