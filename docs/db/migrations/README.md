...
| 012 | `012_backfill_missing_room_conversations.sql` | ⏳ Prepared, pending live application (NOT yet run) | Study Room → Conversation creation invariant fix (2026-08-18). 004 backfilled one `room`-type Conversation per Study Room that existed at the time, but `StudyRoomsService.create()` was never updated to create one for rooms created afterward — every Study Room created since 004 (until the same-commit application fix) has had NO Conversation at all, breaking chat/attachments/meeting-token for it even though other room operations worked fine. This migration backfills the missing `room`-type Conversation for every currently-orphaned `study_rooms` row (active AND soft-deleted — see `STUDY_PLATFORM_DATABASE_SPEC.md` § 16 for why deleted rooms are included). It does NOT add a new unique constraint: `conversations_room_id_key` (partial unique index on `conversations.room_id`) and `conversations_type_target_check` already exist from 004 and already guarantee at most one `room`-type Conversation per room — 012_preflight.sql confirms both are present before backfilling. The application-side fix (`StudyRoomsService.create()` now creates Room + host `study_room_members` + Conversation atomically in one transaction, mirroring `ChannelsService.create()`) is what prevents this gap from reopening for future rooms — see `app/study_rooms/services/study_room_service.py`. |
...
## Running 012 (NOT yet applied live — read this before running anything)

012 has NOT been run against live Supabase. Do not run
`012_backfill_missing_room_conversations.sql` automatically or without human
review of the preflight output first.
