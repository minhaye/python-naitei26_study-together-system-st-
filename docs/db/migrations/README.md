...
2026-08-19 (014, 015, 016) — update this table whenever a migration is actually run live.
...
| 016 | `016_create_group_notes.sql` | ✅ Applied live, verified | Creates `group_notes`: shared, Group-scoped Notes backing `app/notes/` (Owner/Moderator: read/create/edit/delete; Member: read-only; non-member/inactive membership: no access — enforced via `is_active_group_member`/`is_group_manager`, not note authorship). An earlier draft scoped Notes to `study_rooms` (`room_notes`/`room_id`); it was corrected to the `groups`-scoped design below before ever being applied, so no `room_id`/`room_notes` schema or data ever existed to migrate away from. `title`/`content` length limits (100/2000 chars) are enforced in the Pydantic DTO, not as DB constraints. RLS enabled with zero policies for `authenticated` — FastAPI (`postgres` role) is the sole reader/writer, same precedent as invitations' write side (013); Notes has no Realtime/direct-client access. `016_verify.sql` confirmed live: table and all 7 columns present, `idx_group_notes_group_id` index present, RLS enabled, no `authenticated` policy on `group_notes`, `room_notes` absent (confirms the discarded draft left no trace), 0 rows immediately after (expected — nothing had used the API against this DB yet). |

Full design/rationale for 004-006: `docs/db/STUDY_PLATFORM_DATABASE_SPEC.md` § 12–15. For 007: § 37. For 008: § 8, § 34. For 009: § 9. For 010/011: § 16, § 22. For 012: § 14. For 013: `docs/invitations.md`. For 014/015: § 24. For 016: `app/notes/` (see the row above; no separate design doc exists for Notes).
