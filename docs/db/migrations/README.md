...
2026-08-19 (014, 015, 019), 2026-08-21 (023, 024), 2026-08-22 (025, 028, 029),
2026-08-23 (030), 2026-08-26 (034) — update this table whenever a migration is
actually run live.
...
| 025 | `025_add_message_reactions.sql` | ✅ Applied live, verified | Creates `message_reactions` (Messenger-style emoji reactions on chat messages -- Channel/Study Room/Direct alike, via the same `messages`/`conversations` spine as everything else). One row per `(message_id, user_id)`; picking a new emoji replaces the previous one (`ON CONFLICT DO UPDATE`, see `MessagesService.set_reaction`). `conversation_id` is denormalized from the message purely so Realtime subscribers can filter by it the same way `messages` already is. `REPLICA IDENTITY FULL` so a Realtime DELETE event's `old` payload carries `message_id` (default replica identity only ships the primary key on delete). RLS mirrors `messages_select` exactly (`can_access_conversation`); added to the `supabase_realtime` publication with all of INSERT/UPDATE/DELETE (unlike `messages`, which only ever needed INSERT). Purely additive, no expand/contract split needed. `025_verify.sql` confirmed live 2026-08-22: table/unique constraint/CHECK/`REPLICA IDENTITY FULL`/RLS/`message_reactions_select` policy/no authenticated write policy/realtime publication all OK, 0 rows immediately after. (`025_verify.sql`'s unique-constraint check needed a `column_name::text` cast fix post-write for an `information_schema.sql_identifier[] = text[]` operator error -- file already reflects the fix.) |
| 026 | `026_enable_study_room_members_realtime.sql` | ✅ Applied live 2026-08-22 | Fix for a reported bug: an already-open Study Room tab's member list/count only ever refreshed after the CURRENT user's own join/leave/moderate action, so it silently went stale when someone else joined/left (client-side REST-polling fallback added the same session, `useStudyRoom.ts`, 10s interval, kept as a resilience fallback alongside Realtime). Adds `study_room_members_select_room_participant` (SELECT, PERMISSIVE, scoped to the pre-existing `can_access_room_conversation(room_id)` -- no new RLS function introduced), then adds `study_room_members` to the `supabase_realtime` publication. Live verify run surfaced FOUR pre-existing `authenticated` policies on this table that had never been captured in this repo (010's note flagged this table's base policies as unknown) -- including an INSERT/UPDATE/DELETE write bypass and a broader SELECT policy that made this migration's own new policy a redundant no-op. Both findings are fixed by 027, immediately below -- read that row too. (`026_verify.sql` also had a latent bug fixed post-write: `pg_policies.cmd` returns spelled-out values like `'SELECT'`, not the single-char codes `'r'/'a'/'w'/'d'` -- the original checks compared against `'r'` and always failed regardless of actual state; file now compares against `'SELECT'`. The identical bug existed in `020_verify.sql`, also fixed, that migration still not yet run live.) |
| 027 | `027_lock_down_study_room_members_writes.sql` | ✅ Applied live 2026-08-22, verified | Follow-up fix for the two gaps 026's live verify run surfaced (see 026's row). (1) **Write bypass, the serious one**: drops `study_room_members_insert`/`_update`/`_delete` (`authenticated`, `WITH CHECK`/`USING`: `user_id = auth.uid() OR is_room_manager(room_id)`) -- these let any authenticated user write their own membership row directly via Supabase/PostgREST, bypassing `join_room`/`leave_room`'s FastAPI checks entirely, and let anyone `is_room_manager(room_id)` (an uncaptured, unaudited legacy function -- also used by `room_moderation_actions`' `room_moderation_select` policy, see 007, out of this migration's scope) returns true for write ANY member's row directly, bypassing `can_manage_room`'s current-Group-role check. A repo-wide grep confirmed nothing in the frontend writes to `study_room_members` directly (only FastAPI does, via a Postgres role that bypasses RLS) -- these three policies were a bypass with no legitimate caller, brings the table in line with this repo's post-013 "FastAPI is the sole writer" convention. (2) **Read parity gap**: drops the old, broader `study_room_members_select` (any active Group member, no room-participation check) so 026's correctly-scoped `study_room_members_select_room_participant` becomes the sole SELECT policy -- mirrors the exact class of fix migration 011 made for `can_access_room_conversation()`. `027_preflight.sql` captured the exact bodies dropped for the record before running. Purely subtractive (policy objects only, no data touched) -- no rollback script, same reasoning 013 used for dropping its own stale write policies. `027_verify.sql` confirmed live: the three write policies and the old broad SELECT policy are gone, `study_room_members_select_room_participant` is the sole remaining SELECT policy (PERMISSIVE, `authenticated`, scoped to `can_access_room_conversation(room_id)`), `study_room_members_block_when_room_deleted` (010's RESTRICTIVE policy) untouched, RLS still enabled, table still in the `supabase_realtime` publication, row count unchanged (60). (`027_verify.sql`'s `no_authenticated_write_policy` check needed a `permissive = 'PERMISSIVE'` fix post-write -- it was incorrectly flagging `study_room_members_block_when_room_deleted` itself, a RESTRICTIVE policy that only narrows access and was never a grant; file already reflects the fix.) |
| 028 | `028_add_forum_reactions.sql` | ✅ Applied live 2026-08-22 | Expand phase for the Forum multi-emotion reactions feature (`feat/forum-reactions`) -- replaces the old boolean-only `post_likes`/`comment_likes` "Thích" button with the same Messenger-style emoji-reaction shape `message_reactions` (025) already uses: `post_reactions`/`comment_reactions`, one row per `(post_id\|comment_id, user_id)`, `emoji` CHECK-constrained to the same 6-emoji set (`👍❤️😆😮😢😡`), picking a new emoji replaces the previous one (`ON CONFLICT DO UPDATE`, see `ForumService.set_post_reaction`/`set_comment_reaction`). Backfilled every existing `post_likes`/`comment_likes` row as a `👍` reaction so no engagement data was lost. RLS enabled on both new tables with zero `authenticated` policies (FastAPI remains the sole reader/writer, same as the rest of Forum -- no Realtime/direct-client access exists here, unlike chat). |
| 029 | `029_drop_forum_likes.sql` | ✅ Applied live 2026-08-22 | Contract phase for 028 -- dropped `post_likes`/`comment_likes` now that the accompanying backend change (`app/forum/`) reads/writes `post_reactions`/`comment_reactions` exclusively (same 004/005 two-step this repo already used for `messages.channel_id`). `029_preflight.sql` anti-joined `post_likes`/`comment_likes` against `post_reactions`/`comment_reactions` to confirm 028's backfill fully covered every row before the drop. No rollback script: reverting would mean recreating the tables and re-deriving rows from `post_reactions`/`comment_reactions` (lossy for any non-`👍` reaction), not worth scripting for a change this size (same reasoning 008/009/012/013 used to skip a rollback). |
| 030 | `030_add_forum_moderation.sql` | ✅ Applied live 2026-08-23 (per user confirmation, not independently re-verified from this environment -- no Supabase credentials available here) | Forum Moderator + Ban system. Adds `profiles.role` (`profile_role` enum: user/moderator/admin -- the first-ever site-wide role in this app; every prior role was Group/Room-scoped), `user_bans` (one row per (user, ban_type) grant -- **5** independent ban types: post/comment creation, messaging, group creation, group joining, and study room joining (`join_room`, added after this migration was first written but before it was run live -- "active" is evaluated in application code, not a partial index, since it depends on `now()`), `forum_moderation_actions` (unified audit log for deletes/bans/unbans/role grants), and `forum_posts.deleted_by` (same soft-delete-attribution convention as `channels.deleted_by`/`study_rooms.deleted_by`, 009/010). RLS enabled on both new tables with zero `authenticated` policies (FastAPI remains the sole reader/writer, same as the rest of Forum -- see 028's note). Purely additive, no contract phase needed. **After running**, bootstrap your own account to `admin` using the commented `update public.profiles set role = 'admin' where id = (select id from auth.users where email = '...')` template at the bottom of the migration file -- there is no other way to create the first Admin, by design (no UI can grant a role nobody has yet). No rollback script: reverting would mean recreating any `user_bans`/`forum_moderation_actions` history that has since accumulated, not worth scripting (same reasoning 008/009/012/013 used). |
| 031 | `031_add_user_reports.sql` | ✅ Applied live 2026-08-23 | User report system (feat/forum-moderation follow-up, `report_reason`/`report_status` enums + `user_reports`, one row per report a user files against another user via the new "Báo cáo" popover item). Purely additive, no contract phase needed. RLS enabled with zero `authenticated` policies (FastAPI remains the sole reader/writer, same as `user_bans`/`forum_moderation_actions`). 030 is already live, so this had to be its own migration rather than an edit to 030. Run `031_preflight.sql`, then this migration, then `031_verify.sql`. No rollback script (same reasoning 008/009/012/013/030 used — nothing to lose before this has ever run live). |
| 032 | `032_add_study_room_presentation_state.sql` | ✅ Applied live 2026-08-23 | Adds `study_rooms.presentation_state` JSONB (feat/realtime-whiteboard), same shape/precedent as `whiteboard_state` (see note below this table -- that column's own migration is filed under `017_add_study_room_whiteboard_state.sql`, a duplicate `017` number with `017_create_profile_avatars_bucket.sql`; never reconciled or tracked in this table before now). Backs the "Slide Bài giảng" synced presentation-deck feature: `{asset_path, file_name, page, page_count}` written only via `PUT /study-rooms/{room_id}/presentation` (host/moderator only, `can_edit_whiteboard`). Purely additive. Run `032_preflight.sql`, then this migration, then `032_verify.sql`; `032_rollback.sql` provided (simple column drop, nothing to lose before this has ever run live). |
| 033 | `033_create_group_streaks.sql` | ✅ Applied live 2026-08-24 | Gamify group engagement by tracking daily message and study room activity. Adds `group_streaks` table and triggers on `messages` and `study_room_members` to auto-calculate the streak passively. |
| 034 | `034_consolidate_forum_categories.sql` | ✅ Applied live 2026-08-26, verified | Data-only cleanup (no schema change): consolidates the 52 `forum_categories` rows accumulated by `scripts/seed_forum.py`/`scripts/seed_rich_data.py` down to a fixed set of 19 school/university study subjects (see `docs/db/STUDY_PLATFORM_DATABASE_SPEC.md` § 25 for the table shape). 18 rows were kept as "anchors" (13 renamed in place, 5 already had their final name) preserving their `id`; one final category (`Kế toán & Kiểm toán`) had no existing row and was inserted fresh; the other ~33 rows were either merged into an anchor (their posts' `category_id` reassigned first, since `forum_posts.category_id` is `ON DELETE RESTRICT`) or, for the 4 with zero posts and no reasonable fit (Âm Nhạc & Cảm Âm, Góc Tuyển Dụng & Thực Tập Sinh, Săn Học Bổng Du Học 2026, Thể Thao & Sức Khỏe Học Đường), dropped outright. Run via `034_preflight.sql` → `034_consolidate_forum_categories.sql` → `034_verify.sql` (this session had live `DATABASE_URL` credentials and no `psql`/Supabase CLI available, so the statements were executed through a one-off SQLAlchemy async-engine script rather than the SQL Editor — same statements, same transaction semantics). `034_verify.sql`-equivalent checks confirmed live: exactly 19 categories, names match the final list byte-for-byte, `forum_posts` count unchanged at 105 (0 orphaned, 0 null `category_id`). Both seed scripts were updated in the same commit to seed/top-up only the final 19 names on a fresh or partial DB, so re-running them does not resurrect the removed categories. No rollback script (same reasoning 008/009/012/013 used — reconstructing which merged category each post originally belonged to isn't recoverable from the post-migration state). |
