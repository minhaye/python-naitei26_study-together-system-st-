-- Preflight checks for 014_create_forum_tags.sql.
...
with
  ...
  tags_table_absent as (
    select
      'tags_table_absent (must be OK/none-found -- 014 creates this table)' as check_name,
  ...
  post_tags_table_absent as (
    select
      'post_tags_table_absent (must be OK/none-found -- 014 creates this table)' as check_name,
      case when count(*) = 0 then 'OK/none-found' else 'FAIL/already-exists' end as status,
      count(*)::text as detail
    from information_schema.tables
