...
if TYPE_CHECKING:
    ...
    from app.models.forum import Comment, CommentLike, ForumPost, PostLike
...
class Profile(Base):
    """Maps to public.profiles. `id` also references auth.users(id) (Supabase-managed
    auth schema); that table is out of scope here, so no FK/model is declared for it."""
    ...
    created_channels: Mapped[list["Channel"]] = relationship(back_populates="creator")
