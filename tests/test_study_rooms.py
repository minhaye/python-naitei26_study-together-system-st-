...
def _no_active_bans(monkeypatch):
    """join_room now checks for an active JOIN_ROOM ban on every join -- default every test
    to "not banned" so unrelated tests don't have to know about the moderation feature. Tests
    that want to exercise a ban override this per-test via monkeypatch."""
